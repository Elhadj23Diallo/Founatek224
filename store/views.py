...
from .models import Category, Product, Cart, CartItem, Order, OrderItem, LoyaltyAccount
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if discount > 0 and use_points > 0:
            ...
            messages.info(request, f'{use_points} points utilisés → -{discount:,} GNF de remise.'.replace(',', ' '))
...
def loyalty_dashboard(request):
    ...
    return render(request, 'store/loyalty.html', {'loyalty': loyalty, 'transactions': transactions})
...
def order_receipt_pdf(request, pk):
    ...
    info_data = [
        ...
        [
            ...
            Paragraph(
                f'Date : {now.strftime("%d/%m/%Y à %H:%M")}<br/>'
    ...
    footer_data = [[
        Paragraph(
            ...
            '<font size=8 color="#555">Orange Digital Center, Ratoma, Conakry — Guinée</font>',
    ...
    story.append(Paragraph(
        f'Document généré le {timezone.localtime(timezone.now()).strftime("%d/%m/%Y à %H:%M")}',
        s_small
    ))
